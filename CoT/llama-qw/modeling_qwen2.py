...
#! 均方根归一化层，用于对输入进行归一化处理，类似于T5LayerNorm。
class Qwen2RMSNorm(nn.Module):
    ...
    def extra_repr(self):
        # 返回权重参数的形状和方差epsilon的值
        return f"{tuple(self.weight.shape)}, eps={self.variance_epsilon}"


# Copied from transformers.models.llama.modeling_llama.LlamaRotaryEmbedding with Llama->Qwen2

#! 旋转位置嵌入层，用于为输入序列中的每个位置生成位置嵌入。它支持动态更新频率，以适应不同长度的序列。
class Qwen2RotaryEmbedding(nn.Module):
    def __init__(
        ...
        # inv_freq 的含义是频率的倒数，用于计算旋转位置编码
        # 将inv_freq注册为缓冲区，使其在模型保存和加载时保持不变
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        ...
        if seq_len > self.max_seq_len_cached:  # growth
            # 如果序列长度超过了缓存的最大序列长度，则重新计算inv_freq
        ...
        if seq_len < self.original_max_seq_len and self.max_seq_len_cached > self.original_max_seq_len:  # reset
            # 如果当前序列长度小于原始最大序列长度，且缓存的最大序列长度大于原始最大序列长度，则重置inv_freq
            self.register_buffer("inv_freq", self.original_inv_freq, persistent=False)
            self.max_seq_len_cached = self.original_max_seq_len

    @torch.no_grad()
    def forward(self, x, position_ids):
        ## position_ids 表示当前到达第几个输出token了
        # print(position_ids) #?????
        # print(f"position_ids.shape = {position_ids.shape}")
        # 如果rope_type中包含"dynamic"，则更新动态频率
        ...
        # 如果设备类型是字符串且不是"mps"，则将设备类型设置为"cpu"
        device_type = device_type if isinstance(device_type, str) and device_type != "mps" else "cpu"
        # 使用torch.autocast将设备类型设置为device_type，并禁用autocast
...
def apply_hybrid_position_emb(q, k, cos, sin, length, position_ids=None, unsqueeze_dim=1):
    ...
    # 分割q和k为两部分，每部分64维
    q_part1, q_part2 = q[..., :64], q[..., 64:]
    k_part1, k_part2 = k[..., :64], k[..., 64:]

    # 对第一部分应用RoPE位置编码
    q_part1_embed, k_part1_embed = apply_rotary_pos_emb(q_part1, k_part1, cos[..., :64], sin[..., :64], position_ids, unsqueeze_dim)

    # 对第二部分应用反向位置编码
    q_part2_embed, k_part2_embed = apply_reverse_position_emb(q_part2, k_part2, cos[..., 64:], sin[..., 64:], length, position_ids, unsqueeze_dim)

    # 合并两部分
    q_embed = torch.cat((q_part1_embed, q_part2_embed), dim=-1)
    k_embed = torch.cat((k_part1_embed, k_part2_embed), dim=-1)

    return q_embed, k_embed



def apply_reverse_position_emb(q, k, cos, sin, length, position_ids=None, unsqueeze_dim=1):
    """Add reverse position embedding to the query and key tensors."""
    ...
    # 获取q_embed和k_embed所在的设备
    device_q = q_embed.device
    device_k = k_embed.device
    ...
    # 将k_embed乘以delta_array_expand
    k_embed = k_embed * delta_array_expand
...
def apply_lrpe_position_emb(q, k, cos, sin, length, position_ids=None, unsqueeze_dim=1):
    ...
    # 计算 LRPE
    lrpe_q = _compute_lrpe(position_ids, length, q.size(-1))
    lrpe_k = _compute_lrpe(position_ids, length, k.size(-1))

    # 将 LRPE 叠加到 query 和 key 上
    q_embed = q_embed + lrpe_q
    k_embed = k_embed + lrpe_k
...
def _compute_lrpe(positions, length, dim):
    ...
    # 计算频率
    freqs = torch.einsum("ij,k -> ijk", t, inv_freq)  # [batch_size, seq_len, dim/2]
    # print(f"freqs.shape = {freqs.shape}") ## ([1, 1, 64])
    cos = freqs.cos()  # [batch_size, seq_len, dim/2]
    sin = freqs.sin()  # [batch_size, seq_len, dim/2]
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        # 获取输入张量的batch size, query长度和隐藏层大小
        bsz, q_len, _ = hidden_states.size()

        # 将输入张量分别投影到query, key和value空间
        query_states = self.q_proj(hidden_states)
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)

        # 将query, key和value张量分别reshape为(batch size, query长度, num heads, head dim)的形状，并进行转置
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)

        # 如果没有提供position_embeddings，则计算RoPE embeddings
        ...
        # 如果past_key_value不为None，则更新key_states和value_states
        if past_key_value is not None:
            # 为RoPE模型指定参数
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}  # Specific to RoPE models
            # 更新key_states和value_states
...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        if position_embeddings is None:
            ...
            cos, sin = position_embeddings ## 先在进入该attention模块之前计算出position embeddings，再进入apply直接运用
            
        # print(f"position_ids.shape in attn is {position_ids.shape}, device is {position_ids.device}")
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        # 如果启用了滑动窗口注意力，但注意力实现不是flash_attention_2，则发出警告
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 对hidden_states进行输入层归一化
        hidden_states = self.input_layernorm(hidden_states)

        # Self Attention
        # 对hidden_states进行自注意力机制，得到新的hidden_states、self_attn_weights和present_key_value
        ...
        # 保存原始的hidden_states
        residual = hidden_states
        # 对hidden_states进行输出层归一化
        hidden_states = self.post_attention_layernorm(hidden_states)
        # 对hidden_states进行全连接层
        ...
        # 如果需要输出注意力权重，则将self_attn_weights添加到outputs中
        if output_attentions:
            outputs += (self_attn_weights,)

        # 如果需要缓存，则将present_key_value添加到outputs中
        if use_cache:
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        if cache_position is None:
            ...
            # 创建一个从past_seen_tokens到 past_seen_tokens + inputs_embeds.shape[1]的序列，并将其放在inputs_embeds的设备上
    ...
    def _update_causal_mask(
        ...
        # 如果使用的是flash_attention_2，则直接返回attention_mask
...
#! 用于因果语言建模的Qwen2模型，包含一个线性层用于生成下一个词的概率分布。
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        # 初始化Qwen2ForCausalLM模型
        super().__init__(config)
        # 初始化Qwen2模型
        self.model = Qwen2Model(config)
        # 获取词汇表大小
        self.vocab_size = config.vocab_size
        # 初始化线性层，用于生成下一个词的概率分布
    ...
    # 添加模型前向函数的文档字符串
    @add_start_docstrings_to_model_forward(QWEN2_INPUTS_DOCSTRING)
    # 替换返回文档字符串，输出类型为CausalLMOutputWithPast，配置类为_CONFIG_FOR_DOC
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        # print(self.model._attn_implementation)
...
#! 用于序列分类的Qwen2模型，包含一个线性层用于计算分类得分。
...
#! 用于标记分类的Qwen2模型，包含一个线性层用于计算每个标记的分类得分。
...
#! 用于问答任务的Qwen2模型，包含两个线性层用于计算答案的开始和结束位置。
