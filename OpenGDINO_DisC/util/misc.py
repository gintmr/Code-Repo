...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, logger=None):
        # 如果logger为空，则使用print函数，否则使用logger.info函数
        if logger is None:
            print_func = print
        else:
        ...
        # 初始化SmoothedValue对象，用于计算平均时间
        iter_time = SmoothedValue(fmt='{avg:.4f}')
        # 初始化SmoothedValue对象，用于计算平均数据时间
        data_time = SmoothedValue(fmt='{avg:.4f}')
        # 计算迭代次数的字符串长度
        space_fmt = ':' + str(len(str(len(iterable)))) + 'd'
        # 如果有可用的GPU，则使用GPU的日志信息
        ...
        for obj in iterable:
            ...
            # 如果i是print_freq的倍数或者i是iterable的最后一个元素，则打印日志信息
            if i % print_freq == 0 or i == len(iterable) - 1:
                # 计算剩余时间
                eta_seconds = iter_time.global_avg * (len(iterable) - i)
                # 将剩余时间转换为字符串
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                # 如果有可用的GPU，则打印GPU的日志信息
                if torch.cuda.is_available():
...
# 定义一个函数，用于将batch中的数据按照指定的方式组合
def collate_fn(batch):

    # 将batch中的数据按照指定的方式组合
    batch = list(zip(*batch))
    # 将batch中的第一个数据转换为嵌套张量
...
class NestedTensor(object):
    def __init__(self, tensors, mask: Optional[Tensor]):
        # 初始化NestedTensor类，传入tensors和mask参数
        self.tensors = tensors
        self.mask = mask
        # 如果mask为'auto'，则根据tensors的维度生成mask
        if mask == 'auto':
...
def setup_distributed(args):
    # 检查环境变量中是否存在WORLD_SIZE，如果存在且不为空，则设置分布式训练参数
    if 'WORLD_SIZE' in os.environ and os.environ['WORLD_SIZE'] != '': # 'RANK' in os.environ and 
        local_world_size = int(os.environ['WORLD_SIZE'])
        args.world_size = args.world_size * local_world_size
        args.gpu = args.local_rank = int(os.environ['LOCAL_RANK'])
        ...
        print('world size: {}, rank: {}, local rank: {}'.format(args.world_size, args.rank, args.local_rank))
        print(json.dumps(dict(os.environ), indent=2))
    # 如果环境变量中存在SLURM_PROCID，则设置分布式训练参数
    elif 'SLURM_PROCID' in os.environ:
        args.rank = int(os.environ['SLURM_PROCID'])
        args.gpu = args.local_rank = int(os.environ['SLURM_LOCALID'])
        ...
        # 如果环境变量中不存在MASTER_PORT，则设置默认端口
        if "MASTER_PORT" not in os.environ:
            os.environ["MASTER_PORT"] = "23233"
        # 如果环境变量中不存在MASTER_ADDR，则设置默认地址
        ...
        print('world size: {}, world rank: {}, local rank: {}, device_count: {}'.format(args.world_size, args.rank, args.local_rank, torch.cuda.device_count()))
    # 如果环境变量中既不存在WORLD_SIZE，也不存在SLURM_PROCID，则不使用分布式训练
    else:
        print('Not using distributed mode')
        args.distributed = False
        args.world_size = 1
